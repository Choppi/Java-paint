...
public enum FigureType {
	 ...
	 */
	CIRCLE, ELLIPSE, RECTANGLE, ROUNDED_RECTANGLE, POLYGON, NONE;

	/**
	 * Nombre de figures référencées ici (à changer si on ajoute des types
	 * de figures)
	 */
	 ...
	 *
	 * @param stroke
	 *            la césure du trait (ou pas de trait si null)
	 * @param edge
	 *            la couleur du trait (ou pas de trait si null)
	 * @param fill
	 *            la couleur de remplissage (ou pas de remplissage si null)
	 * @param x
	 *            l'abcisse du premier point de la figure
	 * @param y
	 *            l'ordonnée du premier point de la figure
	 * @return une nouvelle instance correspondant à la valeur de cet enum
	 * @throws AssertionError
	 *             si la valeur de cet enum n'est pas prévue
	 */
	public Figure getFigure(BasicStroke stroke, Paint edge, Paint fill,
			Point2D p) throws AssertionError {
		switch (this) {
		case CIRCLE:
			// return new Circle(...);
			return new Circle(stroke, edge, fill, p, Circle.DEFAULT_RAYON);
		case ELLIPSE:
			// return new Ellipse(...);
	 ...
	 *
	 * @param model
	 *            le modèle de dessin à modifier
	 * @param tipLabel
	 *            le label dans lequel afficher les conseils utilisateur
	...
	public AbstractCreationListener getCreationListener(Drawing model,
		...
		case ROUNDED_RECTANGLE:
			// return new FancyCreationListener(...)
			return new RoundedRectangleCreationListener(model, tipLabel);
		case POLYGON:
			// return new AwsomeCreationListener(...);
	 ...
	 *
	 * @param i
	 *            l'entier à convertir en FigureType
