 ...
 * pour traduire les évènements souris en instructions pour le modèle de
 * dessin lors de la création d'une nouvelle figure.
 * </dl>
 *
 * @author davidroussel
 */
...
public class DrawingPanel extends JPanel implements Observer, MouseListener,
	 ...
	 * important de conserver une taille effective qui puisse être renvoyée
	 * dans la méthode {@link #getPreferredSize()} et modifiée par un
	...
	/**
	 * L'{@link InfoPanel} dans lequel afficher les informations à propos de la
	 * figure sous le curseur.
	 */
	 ...
	 *            le modèle de dessin
	 * @param coordLabel
	 *            le label à mettre à jour avec les coordonnées du curseur de
	 *            la souris
	 * @param infoPanel
	 *            le panneau d'information des figures à mettre à jour avec
	 *            les informations relative à la figure située sous le curseur
	 *            de la souris
	 */
	...
	/**
	 * Mise en place du modèle de dessin. Met en place un nouveau modèle et
	 * s'il est non null ajoute ce panel comme observateur du modèle
	 *
	 * @param drawing
	...
	protected void paintComponent(Graphics g) {
		...
		if (drawingModel != null) {
			 ...
			 * sur le flux (éventuellement filtré) : pour chaque figure du
			 * flux il faut la dessiner (avec sa méthode draw)
			 */
	...
	/**
	 * Mise à jour déclenchée par un {@link Observable#notifyObservers()} :
	 * en l'occurence le modèle de dessin ({@link Drawing}) lorsque celui ci
	 * est modifié. Cette mise à jour déclenche une requête de redessin du
	 * panel.
	 *
	 * @param observable
	...
	public void mouseDragged(MouseEvent e) {
		/*
		 * Déplacement de la souris (btn enfoncé) : MAJ des coordonnées de la
		 * souris dans le coordLabel [Opt]MAJ de l'infoPanel si une figure se
		 * situe sous le curseur
		 */
	...
	public void mouseMoved(MouseEvent e) {
		/*
		 * Déplacement de la souris : MAJ des coordonnées de la souris dans le
		 * coordLabel MAJ de l'infoPanel si une figure se situe sous le curseur
		 */
		refreshCoordLabel(e.getPoint());
		refreshInfoPanel(e.getPoint());
	}
	...
	public void mouseEntered(MouseEvent e) {
		/*
		 * On passe dans l'état où l'on doit mettre à jour des infos On
		 * rafraichit le coordLabel avec la position de e On rafraichit
		 * l'infoPanel "
		 */
		this.sendInfoState = true;
		refreshCoordLabel(e.getPoint());
		refreshInfoPanel(e.getPoint());
	}

	/**
	 * Arrêt du rafraichissement des panneaux d'information et effacement de
	 * ces panneaux lorsque la souris sort du panel.
	 *
	 * @param e
	 *            l'évènement souris associé
	 */
	...
	public void mouseExited(MouseEvent e) {
		/*
		 * on est plus dans l'état où l'on met à jour les infos remettre les
		 * coordonnés dans la barre d'état à x = ___ y = ___ Remettre les
		 * champs de l'infoPanel à vide
		 */
		this.sendInfoState = false;
		refreshCoordLabel(e.getPoint());
		infoPanel.resetLabels();
	 ...
	 * nouvelles coordonnées ou bien avec la {@link #defaultCoordString} si
	 * l'on affiche pas les coordonnées
	 *
	 * @param x
	...
	private void refreshCoordLabel(Point p) {
		/*
		 * Si on est dans l'état où l'on met à jour les infos Formattage des
		 * valeurs de la position de p Mise à jour du coordLabel contenant les
		 * coordonées Sinon On remet coordLabel à sa valeur par défaut
		 */
		if (sendInfoState) {
			double x = p.getX();
			double y = p.getY();
			coordLabel.setText("x: " + coordFormat.format(x) + "y: "
	 ...
	 *
	 * @param p
	 *            la position du curseur pour déclencher la recherche de
	 *            figures sous ce curseur
	 */
	private void refreshInfoPanel(Point2D p) {
		/*
		 * Si on est dans l'état où l'on met à jour les infos On recherche la
		 * dernière figure sous le curseur Si on a trouvé une figure met à
		 * jour l'info panel avec les infos de la figure Sinon On remet
		 * l'infoPanel à ses valeurs par défaut
		 */
		Figure undermouseFigure = null;

		if (sendInfoState) {

			if ((undermouseFigure = drawingModel.getFigureAt(p)) != null) {
				infoPanel.updateLabels(undermouseFigure);
			} else {
				infoPanel.resetLabels();
			}
		}
